...
<div id="usercenter">
    ...
    <div class="inputs">
        <div>
            ...
            <input type="text" v-model.trim="mail" :placeholder="$t('请输入邮箱')" autocomplete="off">
        </div>
        <div>
            ...
            <span v-if="!isgetting" style="position:absolute;z-index:91;right:1.07rem;color:#509fff;font-size:14px;"  id="Countdownvala"  @touchend="sendmailcode">{{$t('发送验证码btn')}}</span>
            <span v-if="isgetting" style="position:absolute;z-index:91;right:1.07rem;color:#999;font-size:14px;" id="Countdownvala" >{{$t('发送验证码btn')}}</span>
        </div>
        <div>
            ...
            <div class="tip-box"  v-show="tipState">
                ...
                <img  src="https://s.lendx.vip/static/test/images/personnal/module1/tip.png"  style="opacity: 0;" alt="tip">
            </div>
            <img id="taparea" @touchend="toShowTip" src="https://s.lendx.vip/static/test/images/personnal/module1/tip.png" alt="tip">
        </div>
        <div v-if="!code">
            <img src="https://s.lendx.vip/static/test/images/personnal/module1/cart4.png" alt="Inviter's ID">
            <input v-model="Inviter" type="text" :placeholder="$t('请输入邀请码')"  autocomplete="off">
        </div>
    </div>
    <div class="confirmbtn" @touchend="confirm">{{$t('立即注册')}}</div>
    <div class="otheract">
        <span><router-link to="/Login" style="color:#509fff;">{{$t('去登录')}}</router-link></span>
    </div>
</div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        confirm() {
            //检测邮箱密码是否正确
            googletotal('mine_singUp',this.$i18n.locale,'点击注册');
            if(!this.mail){
            ...
            this.$axios.post('/trade/account/signup', data).then(res => {
                ...
                if(code==2000){
                    googletotal('mine_singUp_success',this.$i18n.locale,'注册成功');
                    this.login();
                }
            })
        },
