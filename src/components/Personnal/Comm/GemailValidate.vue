    ...
    <div id="coinoperation">
        <div class="header_pl">
            <div class="headerline clearfloat" style="text-align:center;">
                <img id="puttycancel" src="https://s.lendx.vip/static/test/images/icon/pic_navb.png" @touchend="hideValid" alt="back"> 
                <p><slot></slot></p>
            </div>     
        </div>
        <form id="my-form-verif" class="form-comm">
            <div>
                <label for="email">{{$t('b邮箱验证码')}}</label>
                <input name="email" id="myEmail" maxlength="4"  type="text" v-model.trim="emailCode" :placeholder="$t('邮箱验证码')"/>
                <span v-show="!isgetting" @touchend="sendmailcode" style="position: absolute;color:#509fff;right: .4rem;">{{$t('发送验证码btn')}}</span>
                <span v-show="isgetting" id="Countdownvala" style="position: absolute;color:#999;right: .4rem;">{{$t('发送验证码btn')}}</span>
            </div>
            <div>
                <label for="nickname">{{$t('谷歌验证码')}}</label>
                <input name="nickname" v-model.trim="googleCode" maxlength="6" type="text" :placeholder="$t('请输入谷歌验证码')"/>
            </div>
            <input type="button" :value="this.$t('确定')" :disabled="disabled" @touchend="VerifProp($event)"/>
        </form>
   </div>
</template>
<script>
...
.headerline{height:1.17rem;line-height:1.17rem;margin-bottom: 10px;font-size:16px;color:#262626;background-color:#fff;border-bottom:0.5px solid #e6e6e6;}
