...
<div id="_investment">
    <div class="header_pl">
        <div class="headerline">
            <img src="https://s.lendx.vip/static/test/images/icon/pic_navb.png" alt="" @touchend="cancelalert"> 
            <!-- 投资列表 -->
            <p>{{$t('我的投资')}}</p>
        </div>    
        <div class="myinvestlist" v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="10">
            ...
            <div class="myinvesttype" v-for="(item,index) in investList" :key="index">
                <div class="tl_myinvesttype">
                    <div >
                          <p>
                            <router-link :to="'/Investment/Investmentinfo?id='+item.borrowId+'&borrowCryptoId='+item.cryptoId" class="toInvestmentinfo">{{item.orderId}}</router-link>
                        </p>
                        <p>{{item.ctime}}</p>
                    </div>
                ...
                <div class="ct_myinvesttype">
                    <p><span>{{$t('出借金额')}}</span><span>{{item.amount}}&nbsp;{{item.cryptoCode}}</span></p>
                    <p><span>{{$t('利息')}}</span><span>{{parseFloat(item.profit.toFixed(6))}}&nbsp;{{item.cryptoCode}} </span></p>
                    <p><span>{{$t('到期时间')}}</span><span>{{item.deadline}}</span></p>
                    <p><span>{{$t('还款时间')}}</span><span>{{item.cutoffTime}}</span></p>
                </div>
            </div>

            </transition-group> 
            <div class="more_loading" v-show="loading&&investList.length" style="padding: 8px 0;text-align:center;">
            ...
            <!-- 增加暂无数据背景图 -->
            <div class="norecords-default" v-if="!investList.length">
...
.headerline{position: fixed;top:0;width: 100%;z-index:1000;border-bottom: 0.5px solid #e6e6e6;height:1.17rem;line-height:1.17rem;text-align: center;font-size:16px;color:#262626;background-color:#fff;margin-bottom:10px;}
...
.myinvesttype .tl_myinvesttype >.investstatus{
