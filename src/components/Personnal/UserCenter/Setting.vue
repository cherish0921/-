...
<div class="common">
    <div class="clearfloat" v-if="String(phone).length>=6">
        <span>{{$t('手机号')}}</span><span>{{phone}}</span>
    </div>
    <div class="clearfloat" v-if="String(phone).length<6">
        <span>{{$t('手机号')}}</span>
        <span @touchend.prevent="goToSet(1)" style="padding-right:5px;color:#595959;">{{$t('s_设置')}}</span>
        <img src="https://s.lendx.vip/static/test/images/icon/arrow2.png" alt="Set Up">
    </div>
    <div class="clearfloat" v-if="nickname">
        <span>{{$t('昵称')}}</span><span>{{nickname}}</span>
    </div>
    <div class="clearfloat" v-if="!nickname">
        <span>{{$t('昵称')}}</span>
        <span @touchend.prevent="goToSet(2)" style="padding-right:5px;color:#595959;">{{$t('s_设置')}}</span>
        <img src="https://s.lendx.vip/static/test/images/icon/arrow2.png" alt="Set Up">
    </div>
...
#user .nocountbtn{margin:.75rem 2rem;height:1.17rem;line-height:1.17rem;border:0.5px solid #509fff;color:#509fff;}
</style>
