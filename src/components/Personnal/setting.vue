...
<div id="setting">
    <div class="header_pl">
        <div class="headerline">
            <img src="https://s.lendx.vip/static/test/images/icon/pic_navb.png" @touchend="goBack" alt=""> 
            <p>{{$t('设置')}}</p>
        </div> 
    </div>
    <div  v-if="hasloginls" class="common">
        <div class="clearfloat" @touchend="toModifyPass">
            <span>{{$t('修改密码')}}</span><img src="https://s.lendx.vip/static/test/images/icon/arrow.png" alt="Reset">
        </div>
    </div>
   <div  class="common">
     <div class="clearfloat" @touchend="toModifyLang">
            <span>{{$t('语言')}}</span><img src="https://s.lendx.vip/static/test/images/icon/arrow.png" alt="Language">
        </div> 
    </div>
    <div v-if="hasloginls" class="quitapp" @touchend="quitapp">{{$t('退出登录')}}</div> 
</div>
</template>
<script>
...
#setting .common>div{height:1.33rem;line-height: 1.33rem;background-color:#fff;border-bottom:0.5px solid #e5e5e5;padding-right:.4rem;}
...
#setting .nocountbtn{margin:.75rem 2rem;height:1.17rem;line-height:1.17rem;border:0.5px solid #509fff;color:#509fff;}
