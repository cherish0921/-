    ...
    <transition name="lendchainfade1">
        <div>
            <div class="toumingshade" v-if="toumingshow" >
                <img v-if='type=="b"' src="https://s.lendx.vip/static/test/images/icon/trueoptb@2x.png" alt=""> 
                <img v-if='type=="i"' src="https://s.lendx.vip/static/test/images/icon/trueopti@2x.png" alt="">
            </div>
            <header class="clearfloat">
                <img @touchend="rollback" src="https://s.lendx.vip/static/test/images/icon/pic_navb.png" alt="" srcset="">
                {{title}}
            </header>
            ...
            <div class="content_gauth">
                ...
                <div class="countrycode" v-if="type==1" style="width: 10rem;margin-left: 0;height: 1.33rem;line-height: 1.33rem;font-size: 14px;
                    background-color: #fff;text-align:left;border-bottom: 0.5px solid #e6e6e6;">
...
export default {
    ...
    mounted() {
        switch (Number(this.type)) {
            case 0: //
                this.title = "生成LendChain账户"
                break;
            case 1: //
                this.title = '绑定LendChain账户'
                break;
            case 2: //
                this.title = '设置昵称'
                break;
    ...
    methods: {
        ...
        sendcode() {
            let url = Number(this.type)==3?'/trade/sms/send-code':'/trade/mail/blocksend';
        ...
        getUserInfo(){  //通过账号密码方式登录并父组件回调调用用户信息接口存进cookie
        ...
        confirm() {
            if (this.type == 0) { //生成LendChain账号
                ...
                if (setname.length <= 0) {
                    ...
                    layerid1 = this.$MessageBox.alert( '昵称一旦设置，无法修改。是否确定设置您的昵称为' + setname + "?", {
