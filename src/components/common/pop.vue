...
<div>
    ...
    <div id="popcps">
        <div class="header_pop">
            <span class="popbtn" @touchend="cancelpop">{{this.$t('取消')}}</span>
            <span>{{title}}</span>
            <span class="popbtn"  @touchend="confirmlpop">{{this.$t('确定')}}</span>
        </div>
        ...
        <ul class="poplist" @touchend="scrollend">
            ...
            <li v-if="type=='d'" v-for="(item,index) in list" :data-i="index" :key="index">{{item}}{{$t('天')}}</li>
            <li></li>
        </ul>
        <div class="lines">&nbsp;</div>
        </div>
...
#popcps .lines{position:relative;height:40px;top:40px;border-top:0.5px solid #e6e6e6;border-bottom:0.5px solid #e6e6e6;}
