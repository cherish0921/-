...
import {loadLanguageAsync, setlang} from '../assets/i18n/i18n'
...
router.beforeEach((to, from, next) => {
  let lang = null
  if (to.redirectedFrom && to.redirectedFrom.includes('/ko')) {
    lang = 'ko'
    localStorage.setItem('LENDCHAIN_LANGUAGE', lang)
    loadLanguageAsync(lang).then(
      () => next()
    )
  } else if (to.redirectedFrom && to.redirectedFrom.includes('/en')) {
    lang = 'en'
    localStorage.setItem('LENDCHAIN_LANGUAGE', lang)
    loadLanguageAsync(lang).then(
      () => next()
